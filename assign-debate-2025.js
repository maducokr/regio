/**
 * 2025년 꾸리아 주관 토론대회 배정 (연차총친목회/아치에스 꾸리아 형식)
 * - 구분: 실시 / 제목: 토론대회
...
const pool = new Pool({
    ...
    max: 2,
    application_name: 'regio-debate-assign'
});
...
async function main() {
    ...
    try {
        ...
        const datePool = shuffle(yearDatePool());
        if (curiaNames.length > datePool.length) {
            throw new Error(`꾸리아 수가 일자 후보보다 많습니다.`);
        }
        ...
        console.log('\n========== 배정 완료 ==========');
        console.log(`구분: 실시 | 제목: ${TITLE} | 장소: ${PLACE} | 주관: 꾸리아`);
