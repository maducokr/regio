/**
 * 광주(파란색) 기타 생명존중 세목명 변경
 * 기타-생명존중 활동 → 기타-생명존중활동(헌혈,장기기증,자살예방,낙태반대...)
 *
 * 사용: node rename-gwangju-prolife.js [--render]
 */
...
const FROM = '기타-생명존중 활동';
const TO = '기타-생명존중활동(헌혈,장기기증,자살예방,낙태반대...)';
const DESC = '생명존중활동(헌혈,장기기증,자살예방,낙태반대...) (광주)';
...
async function main() {
    ...
    try {
        ...
        if (existingFrom.rows.length && existingTo.rows.length) {
            ...
            await client.query(
                `UPDATE activity_categories
                 SET category_name = $1, category_group = '기타', description = $2
        ...
        console.log(`\n✅ 생명존중 세목명 변경 완료. (${useRender ? 'Render' : 'local'})`);
    } catch (err) {
