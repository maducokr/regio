...
function injectFieldLabelsModule(html) {
    if (html.includes('if (!window.RegioActivityFieldLabels)')) {
        return html;
    }
    const labelsBlock = `<script src="activity-field-labels.js?v=20260822d"></script>
    <script>
    if (!window.RegioActivityFieldLabels) {
${getActivityFieldLabelsSource()}
    }
    </script>`;
    if (html.includes('activity-field-labels.js')) {
        return html.replace(
            /<script src="activity-field-labels\.js\?v=[^"]+"><\/script>/,
            labelsBlock
        );
    }
    return html.replace(
