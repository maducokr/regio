/**
 * 교우돌봄-첫영성체 세목 추가
 * 사용: node add-believer-first-communion.js
 * Render: DATABASE_URL 있으면 해당 DB에 적용
 */
...
async function ensureMapping(client, fieldName, displayName, required) {
    ...
    await client.query(
        `INSERT INTO activity_field_mapping (category_name, field_name, field_display_name, is_required)
         VALUES ($1, $2, $3, $4)`,
        [CAT.name, fieldName, displayName, required]
    );
}
...
async function main() {
    ...
    try {
        ...
        if (mapExists) {
            await ensureMapping(client, '횟수', '횟수(회,단,시간,명)', true);
            await ensureMapping(client, '첫영성체', '첫영성체', false);
