/**
 * 광주(파란색) 영성생활 기타 → 기타기도 세목명 변경
 * 영성생활-기타 → 영성생활-기타기도(묘지참배/선종단원 위한기도, 단장이 지시한 모든기도, 모바일 성경쓰기등)
 *
 * 사용: node rename-gwangju-other-pray.js [--render]
 */
...
const FROM = '영성생활-기타';
const TO = '영성생활-기타기도(묘지참배/선종단원 위한기도, 단장이 지시한 모든기도, 모바일 성경쓰기등)';
const DESC = '기타기도(묘지참배/선종단원 위한기도, 단장이 지시한 모든기도, 모바일 성경쓰기등) (광주)';
...
async function main() {
    ...
    try {
        ...
        if (existingFrom.rows.length && existingTo.rows.length) {
            ...
            await client.query(
                `UPDATE activity_categories
                 SET category_name = $1, category_group = '영성생활', description = $2
        ...
        console.log(`\n✅ 기타기도 세목명 변경 완료. (${useRender ? 'Render' : 'local'})`);
    } catch (err) {
